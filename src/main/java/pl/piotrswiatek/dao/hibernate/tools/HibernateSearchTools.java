package pl.piotrswiatek.dao.hibernate.tools;
...
public class HibernateSearchTools {
    protected static final Log log = LogFactory
            .getLog(HibernateSearchTools.class);
     ...
     */
    public static Query generateQuery(String searchTerm, Class<?> searchedEntity,
                                      Session sess, Analyzer defaultAnalyzer) throws ParseException {
        Query qry = null;
        ...
        if (searchTerm.equals("*")) {
            ...
            try {
                ...
                if (searchedEntity == null) {
                    ...
                    analyzer = txtSession.getSearchFactory().getAnalyzer(
                            searchedEntity);
                }
                ...
                if(log.isDebugEnabled()) {
                    ...
                    log.debug("fnames = " + fnames);
                    log.debug("queries = " + queries);
                    log.debug("analyzer = " + analyzer);
                    log.debug("searchTerm = " + searchTerm);
                    log.debug("qry = " + qry);
                    log.debug("readerAccessor = " + readerAccessor);
                    log.debug("reader = " + reader);
                }

                qry = MultiFieldQueryParser.parse(Version.LUCENE_35, queries,
                        fnames, analyzer);
            } finally {
